// middleware/upload.js - VERSION OPTIMISÉE
"use strict";
...
const UPLOAD_CONFIG = {
  ...
  // File limits
  maxFileSize: Number(process.env.MULTER_FILE_SIZE || 50 * 1024 * 1024), // 50MB par défaut
  maxFiles: Number(process.env.MAX_FILES_PER_REQUEST || 10),
  maxTotalSize: Number(process.env.MAX_TOTAL_UPLOAD_SIZE || 200 * 1024 * 1024), // 200MB total
  
  // Security whitelist - Types MIME autorisés (SVG exclu pour sécurité)
  allowedMimeTypes: [
    'image/jpeg',
    'image/png', 
    'image/webp',
  ...
  allowedExtensions: ['.jpg', '.jpeg', '.png', '.webp', '.gif', '.tif', '.tiff', '.bmp', '.ico'],
...
class UploadUtilities {
  ...
  static async initializeDirectories() {
    ...
    for (const dir of dirs) {
      try {
        ...
        if (!isProd) {
          console.log(`📁 Répertoire upload créé: ${dir}`);
        }
      }
    }
  }
  ...
  static async safeUnlink(filePath, reason = 'cleanup') {
    try {
      if (filePath && fsSync.existsSync(filePath)) {
        ...
        if (!isProd) {
          console.log(`🗑️ Fichier supprimé (${reason}): ${path.basename(filePath)}`);
        }
      }
    } catch (error) {
      if (!isProd) {
  ...
  static async quarantineFile(filePath, reason, metadata = {}) {
    try {
      ...
      if (!isProd) {
        console.warn(`🚨 Fichier mis en quarantaine:`, logEntry);
      }
      
      return quarantinePath;
    } catch (error) {
      if (!isProd) {
        console.error(`❌ Erreur quarantaine: ${error.message}`);
      }
  ...
  static recordUploadMetrics(files, processingTime, requestId) {
    ...
    const metrics = {
      ...
      processingTime,
      throughput: Math.round(totalSize / (processingTime / 1000)), // bytes/sec
      requestId
    };
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    try {
      ...
      const validation = UploadUtilities.validateFilename(originalName);
      if (!validation.valid) {
        UploadUtilities.logUploadEvent('warn', 'Nom de fichier invalide', {
          originalName,
          reason: validation.reason
        }, requestId);
        return cb(new Error(`Nom de fichier invalide: ${validation.reason}`));
      }
      
      // Génération nom sécurisé
      const nameData = UploadUtilities.generateSecureFilename(originalName);
      
      // Enrichissement métadonnées requête
      req.uploadMeta = {
        ...nameData,
        requestId,
      ...
      UploadUtilities.logUploadEvent('debug', 'Nom de fichier généré', {
        ...
        secure: nameData.secureName,
        extension: nameData.extension
      }, requestId);
      
      cb(null, nameData.secureName);
    } catch (error) {
      UploadUtilities.logUploadEvent('error', 'Erreur génération nom', {
        error: error.message
      }, requestId);
      cb(new Error(`Erreur génération nom: ${error.message}`));
    }
  }
});

// ✅ Filtre fichier avec validation stricte
const fileFilter = (req, file, cb) => {
  ...
  try {
    ...
    if (!UPLOAD_CONFIG.allowedMimeTypes.includes(mime)) {
...
const handleMulterError = (err, req, res, next) => {
  ...
  filesToClean.forEach(async (file) => {
    if (file?.path) {
      await UploadUtilities.safeUnlink(file.path, 'multer_error_cleanup');
    }
  });
  ...
  if (err instanceof multer.MulterError) {
    const errorMappings = {
      ...
      'LIMIT_FIELD_KEY': {
        ...
        details: {
          maxLength: UPLOAD_CONFIG.limits.fieldNameSize
        }
      },
      'LIMIT_FIELD_VALUE': {
        status: 400,
        ...
        details: {
          maxLength: UPLOAD_CONFIG.limits.fieldSize
        }
      },
      'LIMIT_FIELD_COUNT': {
        status: 400,
        ...
        details: {
          maxFields: UPLOAD_CONFIG.limits.fields
        }
      },
      'LIMIT_PART_COUNT': {
        status: 400,
        ...
        type: 'TOO_MANY_PARTS',
        details: {
          maxParts: UPLOAD_CONFIG.limits.parts
        }
      }
    };
