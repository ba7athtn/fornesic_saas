...
const config = require('../config'); // Intégration config centralisée
...
// Cache pour analyses fréquentes (TTL via config.cache.analysisTTL)
const analysisCache = new Map();
const CACHE_TTL = Number(config.cache?.analysisTTL) || 5 * 60 * 1000; // 5 minutes
...
/**
 * Analyse unifiée via ForensicService (non destructif)
 * Route d’essai possible
 */
async function analyzeUnified(req, res) {
 ...
 */
async function getComprehensiveAnalysis(req, res) {
 ...
 */
async function initiateForensicAnalysis(req, res) {
  const startTime = Date.now();
  const requestId = crypto.randomBytes(8).toString('hex');
 ...
 */
async function compareForensicAnalyses(req, res) {
  const startTime = Date.now();
  const requestId = crypto.randomBytes(8).toString('hex');
  ...
  try {
    ...
    // Optionnel: caster en ObjectId si nécessaire
    const oid1 = mongoose.Types.ObjectId.isValid(imageId1) ? imageId1 : validateObjectId(imageId1);
    const oid2 = mongoose.Types.ObjectId.isValid(imageId2) ? imageId2 : validateObjectId(imageId2);

    // Récupération parallèle des analyses
    const [analysis1, analysis2, image1, image2] = await Promise.all([
      Analysis.findOne({ imageId: oid1 }).lean(),
      Analysis.findOne({ imageId: oid2 }).lean(),
      Image.findById(oid1).select('originalName hash size exifData').lean(),
      Image.findById(oid2).select('originalName hash size exifData').lean()
    ]);
 ...
 */
async function getForensicStatistics(req, res) {
  try {
    ...
    const [globalStatsArr, riskDistribution, pillarStatsArr, recentActivity] = await Promise.all([
    ...
    const globalStats = globalStatsArr?.[0] || null;
    const pillarStats = pillarStatsArr?.[0] || null;
    ...
    const sessionSummary = {
      ...
      summary: {
        ...
        sessionStarted: analyses.reduce((earliest, a) =>
          ...
          analyses[0].requestedAt || analyses[0].createdAt
        ),
...
function comparePillarFlags(pillar1, pillar2) {
  // Logique de comparaison spécifique par pilier (placeholder conservé)
...
module.exports = {
  ...
  // Utilitaires exportés si nécessaires ailleurs:
  validateObjectId
};
