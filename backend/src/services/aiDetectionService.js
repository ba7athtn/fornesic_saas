...
class AIDetectionService {
    ...
    try {
      const analysis = {
        ...
        analysisMetadata: {
          ...
          // Utilisation de crypto pour un hash d’entrée (évite un warning d’import non utilisé)
      ...
      // Analyses complémentaires (non bloquantes pour le score global ici)
  ...
  async analyzeDeepfake(preprocessedImage) {
    try {
      ...
      let score = 0;
      const indicators = [];
      if (eyeInconsistency > 0.7) { score += 30; indicators.push('EYE_INCONSISTENCY'); }
      if (blinkingArtifacts > 0.6) { score += 25; indicators.push('BLINKING_ARTIFACTS'); }
      if (faceBlending > 0.8) { score += 35; indicators.push('FACE_BLENDING_DETECTED'); }
      if (temporalInconsistency > 0.5) { score += 20; indicators.push('TEMPORAL_INCONSISTENCY'); }
  ...
  async analyzeGAN(preprocessedImage) {
    try {
      ...
      let score = 0;
      const indicators = [];
      if (gridArtifacts > 0.7) { score += 30; indicators.push('GRID_ARTIFACTS'); }
      if (modeCollapse > 0.6) { score += 25; indicators.push('MODE_COLLAPSE_DETECTED'); }
      if (generatorFingerprint.detected) { score += 40; indicators.push(`GENERATOR_FINGERPRINT_${generatorFingerprint.type}`); }
      if (spectralAnomalies > 0.8) { score += 20; indicators.push('SPECTRAL_ANOMALIES'); }
  ...
  async analyzeDiffusion(preprocessedImage) {
    try {
      ...
      let score = 0;
      const indicators = [];
      if (noiseResiduals > 0.6) { score += 25; indicators.push('NOISE_RESIDUALS'); }
      if (attentionArtifacts > 0.7) { score += 30; indicators.push('ATTENTION_ARTIFACTS'); }
      if (diffusionSignature.detected) { score += 35; indicators.push(`DIFFUSION_SIGNATURE_${diffusionSignature.model}`); }
      if (timestepArtifacts > 0.5) { score += 20; indicators.push('TIMESTEP_ARTIFACTS'); }
  ...
  calculateOverallAIScore(analysis) {
    const weights = { deepfake: 0.3, synthetic: 0.25, gan: 0.25, diffusion: 0.2 };
  ...
  async detectFaces(preprocessedImage) { return Math.random() > 0.3; } // 70% visage
  async analyzeEyeInconsistency(preprocessedImage) { return Math.random() * 0.3; }
  async analyzeBlinkingArtifacts(preprocessedImage) { return Math.random() * 0.4; }
  async analyzeFaceBlending(preprocessedImage) { return Math.random() * 0.2; }
  async analyzeTemporalInconsistency(preprocessedImage) { return Math.random() * 0.3; }
  async analyzeSyntheticNoise(preprocessedImage) { return { artificialPattern: Math.random() * 0.5, noiseLevel: Math.random(), coherence: Math.random() }; }
  async analyzeFrequencyDomain(preprocessedImage) { return { anomalyScore: Math.random() * 0.4, spectralConsistency: Math.random(), frequencySignature: 'unknown' }; }
  async analyzeSyntheticTextures(preprocessedImage) { return { perfectionScore: Math.random() * 0.3, repetitionPattern: Math.random(), naturalness: Math.random() }; }
  async analyzeSyntheticColors(preprocessedImage) { return { unrealisticScore: Math.random() * 0.4, colorCoherence: Math.random(), saturationAnomalies: Math.random() }; }
  async detectGridArtifacts(preprocessedImage) { return Math.random() * 0.3; }
  async detectModeCollapse(preprocessedImage) { return Math.random() * 0.2; }
  ...
  async detectSpectralAnomalies(preprocessedImage) { return Math.random() * 0.4; }
  async detectNoiseResiduals(preprocessedImage) { return Math.random() * 0.5; }
  async detectAttentionArtifacts(preprocessedImage) { return Math.random() * 0.3; }
  ...
  async detectTimestepArtifacts(preprocessedImage) { return Math.random() * 0.4; }
  async analyzeStatisticalPatterns(preprocessedImage) { return { pixelDistribution: Math.random(), correlationAnomalies: Math.random() * 0.3, entropyAnalysis: Math.random() }; }
  async analyzeAnatomicalCoherence(preprocessedImage) { return { proportionConsistency: Math.random(), symmetryAnalysis: Math.random() * 0.2, anatomicalRealism: Math.random() }; }

  getIndicatorConfidence(indicator, analysis) {
    // Confiance basée sur le contexte
    const confidenceMap = {
