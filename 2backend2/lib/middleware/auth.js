...
var authRateLimit = rateLimit({
  ...
  legacyHeaders: false,
  // ✅ CORRECTION IPv6 - Utiliser req.ip directement
...
// =====================================
// FONCTION D'AUTHENTIFICATION PRINCIPALE - ✅ CORRIGÉE
// =====================================
...
function auth(req, res, next) {
  ...
  try {
    ...
    // ✅ CORRECTION CRITIQUE - Validation source claim obligatoire
    if (!decoded.source || decoded.source !== 'authController') {
      console.warn('🚨 Token source non autorisée rejeté:', {
        userId: decoded.sub,
        source: decoded.source || 'missing',
        expectedSource: 'authController',
        ip: req.ip,
        endpoint: req.path,
        timestamp: new Date().toISOString()
      });
      return sendAuthError(res, 'UNAUTHORIZED_TOKEN_SOURCE', 'Token provient d\'une source non autorisée', {
        expected: 'authController',
        received: decoded.source || 'missing',
        reason: 'Seuls les tokens générés par authController sont acceptés'
      });
    }
    ...
    res.setHeader('X-Token-Source', 'authController'); // ✅ AJOUT - Confirme source valide
    ...
    if (req.path.includes('/api/analysis') || req.path.includes('/api/reports')) {
      req.chainOfCustodyEntry = {
        ...
        details: {
          ...
          requestId: req.user.requestId,
          authMethod: 'jwt',
          tokenSource: 'authController' // ✅ AJOUT - Traçabilité source
        }
      };
    }
    next();
...
function forensicLogging(req, res, next) {
  ...
  res.send = function (body) {
    ...
    console.log('🔍 Accès API forensique:', {
      ...
      tokenSource: (req.user && req.user.source) || 'unknown', // ✅ AJOUT - Log source token
      method: req.method,
      path: req.path,
